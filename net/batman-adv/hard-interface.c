...
/**
 * batadv_find_real_wireless_iface() - check net_device ethernet address with wireless physical address
 * @net_device: the device to check
 *
 * Return: pointer to wireless physical interface net_device
 */
struct net_device *batadv_find_real_wireless_iface(const void *iface_ether_addr)
{
    ...
    for_each_netdev(&init_net, netdev) {
        if (batadv_compare_eth(netdev->dev_addr, iface_ether_addr)) {
            if (batadv_is_wext_netdev(netdev) || batadv_is_cfg80211_netdev(netdev)) {
                result = netdev;
                break;
            }
        }
				   ...
				   struct net_device *soft_iface)
{
	...
	hard_iface->wireless_phy_iface = batadv_find_real_wireless_iface(hard_iface->net_dev->dev_addr);
	if(hard_iface->wireless_phy_iface) {
		hard_iface->wifi_flags = batadv_wifi_flags_evaluate(hard_iface->wireless_phy_iface);
