 ...
 */
static void
minstrel_ht_update_stats(struct minstrel_priv *mp, struct minstrel_ht_sta *mi, struct ieee80211_tx_info *info)
{
	struct minstrel_mcs_group_data *mg;
	struct minstrel_rate_stats *mrs;
	...
	if (mp->tii_rc && (info != NULL)) {

		if (time_after(jiffies, mp->tii_timer_timeout)) {
			mp->tii_timer_running = 0;
		}


		if (info->status.ack_signal) {
			if ( (info->status.ack_signal-95) <= (s8)((mp->tii_rc&0x00ff0000) >>16)){
				if (!mp->tii_timer_running){
					mp->tii_rc_state = (int16_t)(mp->tii_rc&0xffff);
			}

			if (!mp->tii_timer_running) {
				mp->tii_timer_running = 1;
				mp->tii_timer_timeout = jiffies + 1 * HZ;
			}

			if (mp->tii_rc_state != -1) {
				for (i = 0; i < 4; i++)
					mi->max_tp_rate[i] = mp->tii_rc_state;
				mi->max_prob_rate = mp->tii_rc_state;
				}
			}
		}
	} else {
                      ...
                      void *priv_sta, struct ieee80211_tx_status *st)
{
	...
	if (time_after(jiffies, mi->last_stats_update + update_interval)) {
		update = true;
		minstrel_ht_update_stats(mp, mi, info);
	}
			...
			struct ieee80211_sta *sta, void *priv_sta)
{
	...
	minstrel_ht_update_stats(mp, mi, NULL);
	minstrel_ht_update_rates(mp, mi);
}
				    ...
				    struct dentry *debugfsdir)
{
	...
	mp->tii_rc = (u32) 0;
	debugfs_create_u32("tii_rc", S_IRUGO | S_IWUGO, debugfsdir, &mp->tii_rc);
}
#endif
