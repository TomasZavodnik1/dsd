...
{
	struct macsec_dev *macsec = macsec_priv(dev);
	bool sci_present = macsec_send_sci(&macsec->secy);
	unsigned int extra = macsec->secy.icv_len + macsec_extra_len(sci_present);
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	sci_present = (NULL == data) || (NULL == data[IFLA_MACSEC_INC_SCI]) || !!nla_get_u8(data[IFLA_MACSEC_INC_SCI]);
	mtu = real_dev->mtu - icv_len - macsec_extra_len(sci_present);
	if (mtu < 0)
		dev->mtu = 0;
	else
		dev->mtu = mtu;
			 ...
			 void *ptr)
{
	...
	case NETDEV_CHANGEMTU: {
		...
		list_for_each_entry(m, &rxd->secys, secys) {
			struct net_device *dev = m->secy.netdev;
			bool sci_present = macsec_send_sci(&m->secy);
...
static int __init macsec_init(void)
{
	...
	pr_info("MACsec IEEE 802.1AE with TII patch!\n");
