...
/*
 * Emtop: fix error: phy poweron failed --> -110
 */
static void imx8mp_hsmix_fixup(struct imx8mp_blk_ctrl *bc)
{
	int i;
	u32 val;

	/* Set P=12,M=800,S=4 and must set ICP=2'b01. */
	regmap_read(bc->regmap, IMX8MP_GPR_REG2, &val);
	...
	regmap_write(bc->regmap, IMX8MP_GPR_REG2, val);
	/* wait greater than 1/F_FREF =1/2MHZ=0.5us */
	udelay(1);

	regmap_set_bits(bc->regmap, IMX8MP_GPR_REG3, IMX8MP_GPR_REG3_PLL_RST);
	udelay(10);

	/* Set 1 to pll_cke of GPR_REG3 */
	regmap_set_bits(bc->regmap, IMX8MP_GPR_REG3, IMX8MP_GPR_REG3_PLL_CKE);

	/* Lock time should be greater than 300cycle=300*0.5us=150us */
	regmap_read(bc->regmap, IMX8MP_GPR_REG1, &val);
	for (i = 0; i < 100; i++) {
		regmap_read(bc->regmap, IMX8MP_GPR_REG1, &val);
