...
struct ov5640_dev {
	...
	int pwdn_gpio;
	enum of_gpio_flags pwdn_gpio_flags;
	bool   upside_down;
...
{
	if (enable)
		gpio_set_value_cansleep(sensor->pwdn_gpio,
			!!(sensor->pwdn_gpio_flags & OF_GPIO_ACTIVE_LOW));
	else
		gpio_set_value_cansleep(sensor->pwdn_gpio,
			!(sensor->pwdn_gpio_flags & OF_GPIO_ACTIVE_LOW));
}
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	/*
	 * Emtop: because CSI0 & CSI1 share the same powerdown pin. When it's ocupied by one,
	 * the other will fail on function of_get_named_gpio_flags. So don't allow it 'return'
	 * here.
	 */
	sensor->pwdn_gpio = of_get_named_gpio_flags(dev->of_node, "powerdown-gpios", 0,
		&(sensor->pwdn_gpio_flags));
	if (!gpio_is_valid(sensor->pwdn_gpio)) {
		// return -ENODEV;
	}
	else {
		ret = devm_gpio_request_one(dev, sensor->pwdn_gpio, GPIOF_OUT_INIT_HIGH, "ov5640_pwdn");
		if (ret < 0) {
			dev_warn(dev, "Failed to set power pin, return: %d\n", ret);
			// return ret;
		}
	}
