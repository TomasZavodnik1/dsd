 ...
 */
	...
	case rx_8025t: {
		...
		err = regmap_bulk_read(ds1307->regmap, RX8025T_REG_FLAG, regs, 1);
		if (err) {
			dev_dbg(ds1307->dev, "read error %d\n", err);
			goto exit;
		}
		...
		/* oscillator off?  turn it on, so clock can tick. */
		if (regs[0] & RX8025T_FLAG_VLF) {
			dev_warn(ds1307->dev, "voltage drop detected\n");
		}

		if (regs[0] & RX8025T_FLAG_AF) {
			need_clear = true;
			dev_warn(ds1307->dev, "alarm flag detected\n");
		}

		if (regs[0] & RX8025T_FLAG_TF) {
			need_clear = true;
			dev_warn(ds1307->dev, "timer flag detected\n");
		}

		if (regs[0] & RX8025T_FLAG_UF) {
			need_clear = true;
			dev_warn(ds1307->dev, "update flag detected\n");
		}

		if (need_clear) {
			regs[0] &= ~(RX8025T_FLAG_VLF | RX8025T_FLAG_AF | RX8025T_FLAG_TF
				| RX8025T_FLAG_UF);
			err = regmap_bulk_write(ds1307->regmap, RX8025T_REG_FLAG, regs, 1);
			if (err) {
				dev_dbg(ds1307->dev, "write error %d\n", err);
				goto exit;
			}
		}
		break;
